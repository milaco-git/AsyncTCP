...
static inline bool _init_async_event_queue()
{
    if(!_async_queue)
    {
        /**
        * @note: MILACO modify
        * Modifyed 13/09/2022 MILACO:
        */
        // _async_queue = xQueueCreate(32, sizeof(lwip_event_packet_t *));

        _async_queue = xQueueCreate( ASYNC_TCP_EVENTS_QUEUE_SZ, sizeof(lwip_event_packet_t *) );
        if(!_async_queue)
        {
            return false;
        }
...
{
	return ( e && _async_queue && ( xQueueReceive(_async_queue, e, portMAX_DELAY) == pdPASS) );
}

static inline bool _send_async_event(lwip_event_packet_t ** e)
{
	// return _async_queue && xQueueSend(_async_queue, e, ASYNC_TCP_EVENTS_QUEUE_SEND_TMO) == pdPASS;
	return ( e && _async_queue && ( xQueueSend( _async_queue, e, ASYNC_TCP_EVENTS_QUEUE_SEND_TMO ) == pdPASS ) );
}

static inline bool _prepend_async_event(lwip_event_packet_t ** e)
{
	// return _async_queue && xQueueSendToFront(_async_queue, e, ASYNC_TCP_EVENTS_QUEUE_SEND_TMO) == pdPASS;
...
static bool _remove_events_with_arg(void * arg)
{
    ...
    while(!first_packet)
    {
			...
			else if(xQueueSend(_async_queue, &first_packet, ASYNC_TCP_EVENTS_QUEUE_SEND_TMO) != pdPASS)
			{
				return false;
			}
    }
    ...
    while(xQueuePeek(_async_queue, &packet, 0) == pdPASS && packet != first_packet)
    {
			...
			else if(xQueueSend(_async_queue, &packet, ASYNC_TCP_EVENTS_QUEUE_SEND_TMO) != pdPASS)
...
static bool _start_async_task()
{
	...
	if(!_async_service_task_handle)
	{
		...
		xTaskCreateUniversal(_async_service_task, "async_tcp", ASYNC_TCP_TASK_STACK_DEEPTH, NULL, ASYNC_TCP_TASK_PRIORITY, &_async_service_task_handle, CONFIG_ASYNC_TCP_RUNNING_CORE);
		if(!_async_service_task_handle)
		{
			return false;
		}
